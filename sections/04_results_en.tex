...
\begin{frame}{How I evaluated the detector}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \AubayPoCCard{MSCOCO}{\AubayStatusChip{AubayTeal}{OBJECTS}}{natural images, 80 categories}{\textbf{Invented objects}: CHAIR protocol}
    \column{0.31\textwidth}
      \AubayPoCCard{LV-Hallucinations}{\AubayStatusChip{AubayOrange}{ATTRIBUTES}}{finely labelled descriptions}{\textbf{Colors, textures, context}: main NTP training set}
    \column{0.31\textwidth}
      \AubayPoCCard{TextVQA}{\AubayStatusChip{AubayDanger}{READING}}{documents, signs, labels}{\textbf{OCR-style errors}: business use case}
  \end{columns}
  \vspace{0.4cm}
  \begin{itemize}
    \item Sentence-level detection, scored against fine-grained human labels
    \item Metrics: \textbf{ROC-AUC}, PR-AUC, recall (0.5 ROC-AUC $=$ coin flip)
  \end{itemize}
  \end{enslide}
\end{frame}
...
\begin{frame}{Benchmarking four backends}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.30\textwidth}
      \centering
      \includegraphics[width=0.9\linewidth,height=0.30\textheight,keepaspectratio]{media/cand_meerkat.jpg}\par\vspace{0.08cm}
      {\scriptsize\itshape « \ldots an outdoor \textbf{enclosure} with a tree trunk and some logs. »\par}
      \vspace{0.06cm}
      {\scriptsize\bfseries LV-Hallucinations sample}
    \column{0.66\textwidth}
      ...
      \end{tabular}}
      \par\vspace{0.15cm}
      {\scriptsize\itshape Reference paper (Azachi et al.): NTP-only models 0.59--0.66, LLaVA self-check 0.632 $\Rightarrow$ 0.645 matches the SOTA range.\par}
  \end{columns}
  \vspace{0.3cm}
...
% ============================================================
% S13 — [EN] ASD grid search : the trade-off
% ============================================================
\begin{frame}{Tuning the correction, and refusing the "best" cell}
  \framesubtitle{Grid search over $\alpha$ and $\lambda$, 50 TextVQA images}
  \begin{enslide}
  \begin{columns}[T,onlytextwidth]
    \column{0.52\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{media/asd_param_heatmap_grid.png}
    \column{0.44\textwidth}
      ...
      \begin{tabular}{@{}l c c@{}}
        ...
        \midrule
        Baseline (no ASD) & 0.818 & 0.228 \\
        \rowcolor{AubaySuccess!14}
        \textbf{Chosen} $\alpha{=}0.8,\ \lambda{=}2.0$ & \textbf{0.818} & \textbf{0.250} \\
        \rowcolor{AubayDanger!14}
        "Best" cell $\alpha{=}2.0,\ \lambda{=}5.0$ & 0.091 & 0.069 \\
        \bottomrule
      \end{tabular}}
  \end{columns}
  \end{enslide}
...
% ============================================================
\begin{frame}{But wait, what \emph{is} a hallucination?}
...
\begin{frame}{Limits I stand behind}
  ...
  \centering
  \AubayStatusChip{AubayTeal}{NEXT: DOMAIN-SPECIFIC TRAINING}\hspace{0.15cm}
  \AubayStatusChip{AubayOrange}{DOCVQA}\hspace{0.15cm}
