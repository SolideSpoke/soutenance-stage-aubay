...
\begin{frame}{Du papier de recherche à l'arbitrage de valeur}
  ...
  \begin{enumerate}
    \item \textbf{Explorer large} : l'état de l'art
    \item \textbf{Prototyper en parallèle} : dashboard d'audit \textit{vs} effacement d'hallucinations
    \item \textbf{Arbitrer} : valeur métier, démontrabilité
  \end{enumerate}
  \vspace{0.3cm}
  \centering
...
\begin{frame}{Corriger : piloter les activations du modèle}
  ...
  \begin{itemize}
    ...
    \item Injection $z_l \pm \lambda\,\hat{v}_l$ dans les activations du \textbf{tiers central du décodeur}
    \item Décodage contrastif sur les logits : $\alpha$ dose le contraste, \textbf{recherche manuelle}
  \end{itemize}
  \vspace{0.15cm}
  \centering
...
% ============================================================
% Reveal en 3 temps : opacité nulle hors overlay => géométrie stable dans le resizebox
\tikzset{
...
\begin{frame}{L'architecture finale : \texttt{filter+asd}}
  ...
  \begin{tikzpicture}[
    ...
    % --- Temps 1 : le tronc commun génération + scoring ---
    ...
    % --- Temps 2 : piste OK ---
    \node[greenbox,visible on=<2->] (low)  at (6.8,1.75) {Risque faible\\on conserve};
    \node[greenbox,visible on=<2->]  (kept)  at (6.8,0.55) {Phrase\\conservée};
    % --- Temps 3 : piste ALERTE + texte final ---
    \node[redbox,visible on=<3->]   (high) at (10.6,1.75) {Risque élevé\\on corrige};
    \node[orangebox,visible on=<3->] (asd)   at (10.6,0.55) {ASD\\réduit le biais};
    \node[greenbox,visible on=<3->]  (fixed) at (10.6,-0.65) {Phrase\\corrigée};
    \node[graybox,minimum width=3.1cm,visible on=<3->] (final) at (8.7,-1.8) {Texte final corrigé};
    \draw[arrow] (input) -- (vlm);
    \draw[arrow] (vlm) -- (split);
    \draw[arrow] (split) -- (ntp);
    \draw[softarrow,draw=AubaySuccess,visible on=<2->] (ntp.south) -- (low.north);
    \draw[softarrow,draw=AubayDanger,visible on=<3->] (ntp.south) -- (high.north);
    \draw[arrow,visible on=<2->] (low) -- (kept);
    \draw[arrow,visible on=<3->] (high) -- (asd);
    \draw[arrow,visible on=<3->] (asd) -- (fixed);
    \draw[arrow,visible on=<3->] (kept.south) |- (final.west);
    \draw[arrow,visible on=<3->] (fixed.south) |- (final.east);
    \node[draw=AubaySuccess,line width=1.6pt,rounded corners=7pt,fit=(low)(kept),inner sep=7pt,visible on=<2->,
          label={[font=\small\bfseries,text=AubaySuccess,fill=white,inner sep=2pt,visible on=<2->]west:PISTE OK}] {};
    \node[draw=AubayDanger,line width=1.6pt,rounded corners=7pt,fit=(high)(asd)(fixed),inner sep=7pt,visible on=<3->,
          label={[font=\small\bfseries,text=AubayDanger,fill=white,inner sep=2pt,visible on=<3->]east:PISTE ALERTE}] {};
    % Écho décoratif de l'exemple d'introduction (S02), dernier overlay uniquement
    \node[font=\scriptsize\itshape,text=AubaySlate,align=center,visible on=<3->] at (8.7,-2.55)
      {« un plat bleu » $\rightarrow$ « des mangues et des citrons »};
  \end{tikzpicture}}
  \vspace{0.18cm}
