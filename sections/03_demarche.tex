...
\begin{frame}{Ce que j'ai abandonné : le VLM-juge}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.44\textwidth}
      \AubayMetricCard{8 Go}{VRAM : RTX 2080}{le poste mis à ma disposition}
      \vspace{0.2cm}
      \AubayMetricCard{< 1 \%}{de corrections déclenchées}{le juge ne « voit » presque rien}
      \vspace{0.2cm}
      \begin{alertblock}{Décision}
        \small Abandon. Passage à une intervention \textbf{dans le modèle}, au coût marginal quasi nul.
      \end{alertblock}
  \end{columns}
  \AubayTraceRef{A.2.5}
\end{frame}
...
\begin{frame}{La pipeline NTP en détail}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \column{0.38\textwidth}
      \AubayFocusPanel{Deux flux, un classifieur}{Description NTPs (\textbf{avec} image) vs Comparative NTPs (\textbf{sans} image).\\[0.35em]
      $\rightarrow$ descripteurs statistiques par phrase\\[0.35em]
      $\rightarrow$ classifieur léger (régression logistique).}
  \end{columns}
  \AubayTraceRef{A.1.1}
\end{frame}
...
\begin{frame}{Corriger : piloter les activations du modèle}
  ...
  \begin{itemize}
    ...
    \item $\alpha$ (force du contraste) et $\lambda$ (intensité) : \textbf{recherche manuelle de paramètres} (détail en annexe)
  \end{itemize}
  \vspace{0.15cm}
  \centering
