...
\begin{frame}{Du papier de recherche à l'arbitrage de valeur}
  ...
  \begin{enumerate}
    \item \textbf{État de l'art} : interprétabilité mécaniste, SAE, \textit{circuit tracing}, hallucinations
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      \begin{block}{L'idée naïve}
        \small Un 2\up{e} VLM relit l'image et la description, et « juge » les passages inventés.
      \end{block}
      \vspace{0.2cm}
      {\scriptsize\ttfamily\raggedright
        « Compare the image, the reference title, and the draft.\\
        Return ONLY a comma-separated list of\\
        hallucinated concepts. »\par}
      \vspace{0.2cm}
      \begin{itemize}
        \item \textbf{2 passes} de génération par image
        \item Insoutenable sur GPU 8 Go
        \item Effet plafond : ni prompts ni modèles plus petits n'améliorent
      \end{itemize}
    \column{0.44\textwidth}
      \AubayMetricCard{< 1 \%}{de corrections déclenchées}{le juge ne « voit » presque rien}
      \vspace{0.25cm}
      \begin{alertblock}{Décision}
        \small Abandon. Passage à une intervention \textbf{intra-modèle} (white-box), au coût marginal quasi nul.
      \end{alertblock}
  \end{columns}
  \AubayTraceRef{A.2.5}
\end{frame}
...
\begin{frame}{La pipeline NTP en détail}
  \framesubtitle{Azachi et al., \textit{Leveraging NTPs for Efficient Hallucination Detection in VLMs}}
  \centering
  \includegraphics[width=0.86\linewidth,height=0.66\textheight,keepaspectratio]{media/illustration_ntp.png}
  \vspace{0.25cm}

  \centering
  \begin{minipage}{0.86\textwidth}
    \AubayFocusPanel{Deux flux, un classifieur}{Description NTPs (avec image) \textbf{vs} Comparative NTPs (sans image) $\rightarrow$ descripteurs statistiques $\rightarrow$ régression logistique.}
  \end{minipage}
  \AubayTraceRef{A.1.1}
\end{frame}
...
\begin{frame}{Corriger : piloter les activations du modèle}
  ...
  \begin{itemize}
    \item Calibration contrastive : activations des tokens \textbf{factuels} \textit{vs} \textbf{hallucinés} (labels COCO)
    \item Vecteur de contrôle $\hat{v}_l$ injecté dans le \textbf{tiers central du décodeur} : $z_l \leftarrow z_l \pm \lambda\,\hat{v}_l$
  \end{itemize}
  \vspace{0.15cm}
  \centering
...
\begin{frame}{L'architecture finale : \texttt{filter+asd}}
  ...
  \begin{tikzpicture}[
    ...
    \draw[softarrow] (ntp.south) -- node[pos=0.6,left=0.05cm,font=\scriptsize,text=AubaySuccess] {OK} (low.north);
    \draw[softarrow] (ntp.south) -- node[pos=0.6,right=0.05cm,font=\scriptsize,text=AubayDanger] {alerte} (high.north);
    \draw[arrow] (low) -- (kept);
    \draw[arrow] (high) -- (asd);
